...
import { ArrowLeft, ChevronDown, Calendar, MessageCircle, Check, Wallet, Building2 } from 'lucide-react';
...
const MonthlyReportView: React.FC<MonthlyReportViewProps> = ({ transactions, units, onClose, buildingName, onAddFile }) => {
  ...
  const reportData = useMemo(() => {
    ...
    if (previousDevir !== 0) {
      incomes.unshift({ label: "ÖNCEKİ DÖNEMDEN DEVİR", total: previousDevir });
    }
  ...
  const generateAndHandlePdf = async () => {
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: 'Aylık Bilanço' });
      } else {
        pdf.save(fileName);
      }
      ...
      <div className="sticky top-0 z-[100] -mx-4 px-3 pt-3 pb-3 bg-[#030712]/95 backdrop-blur-3xl border-b border-white/5 shadow-2xl">
        <div className="flex items-center space-x-1.5 w-full">
          ...
          <div className="flex-1 min-w-0 grid grid-cols-2 gap-1.5">
            ...
            <button onClick={() => setShowDatePicker(!showDatePicker)} className="bg-[#1e293b] rounded-2xl h-12 flex items-center justify-between px-3.5 border border-white/10 shadow-lg">
              <span className="text-[15px] font-black text-white uppercase truncate leading-none">{months[selectedMonth].toUpperCase().slice(0,3)} {selectedYear}</span>
              <ChevronDown size={14} className="text-zinc-400 shrink-0" />
            </button>
          </div>

          <button onClick={generateAndHandlePdf} disabled={isProcessing} className="bg-green-600/20 border border-green-500/30 rounded-2xl h-12 px-3.5 flex items-center space-x-1.5 text-green-500 shadow-lg active:scale-95 transition-all shrink-0">
        ...
        {showDatePicker && (
          <div className="absolute top-full right-16 z-[150] mt-1 w-48 bg-[#1e293b] border border-white/10 rounded-xl shadow-2xl overflow-hidden flex max-h-[300px]">
            <div className="w-1/3 border-r border-white/5 bg-black/20 overflow-y-auto no-scrollbar">
               {years.map(y => <button key={y} onClick={() => setSelectedYear(y)} className={`w-full py-3.5 text-[13px] font-black ${selectedYear === y ? 'text-green-400' : 'text-white/30'}`}>{y}</button>)}
            </div>
            <div className="w-2/3 overflow-y-auto no-scrollbar">
               {months.map((m, i) => <button key={m} onClick={() => { setSelectedMonth(i); setShowDatePicker(false); }} className={`w-full py-3 px-4 text-left text-[12px] font-black uppercase border-b border-white/5 last:border-0 ${selectedMonth === i ? 'text-green-400 bg-white/5' : 'text-white/60'}`}>{m}</button>)}
            </div>
          </div>
        )}
      </div>
      ...
      <div className="mt-4 px-1" ref={reportRef}>
        ...
        <div className={`mt-5 flex justify-between items-center py-6 px-8 rounded-[32px] border shadow-2xl ${isPdfMode ? 'bg-slate-50 border-slate-300' : 'bg-[#111827] border-white/10'}`}>
          <div className="flex flex-col">
            ...
            <span className={`text-[10px] font-bold uppercase opacity-40 ${isPdfMode ? 'text-slate-500' : ''}`}>DÖNEM SONU BAKİYE</span>
          </div>
